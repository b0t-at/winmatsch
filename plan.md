...
- P8 hardening: E2E vs test repo, analyzer corpus in CI (compile fixtures with Inno/NSIS/WiX on windows job), docs, v1.0 release, submit winmatsch to winget with itself
  - **Implemented (2026-08-02):** the Windows CI matrix includes a pinned compiled-installer corpus for Inno, NSIS, WiX MSI/Burn, and MSIX, while normal unit tests retain independently encoded hermetic fixtures.
...
- Docs reconciled against the built binary and current code: project count 9→8, plan's stale schema-validation seam marked superseded, ManifestVersion 1.10.0→1.12.0, DRY_RUN precedence, line-ending policy, transaction recovery journals, anonymous public reads, and an explicit "unsupported (not bugs)" table.
